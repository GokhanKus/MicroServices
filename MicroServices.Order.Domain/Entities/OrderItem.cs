namespace MicroServices.Order.Domain.Entities
{
	public class OrderItem:BaseEntity<int>
	{
		...
		{
			if (string.IsNullOrEmpty(ProductName)) throw new ArgumentNullException("product name cannot be empty");
		...
		{
			if(newPrice <= 0) throw new ArgumentOutOfRangeException("unit price must be greater than 0");
			UnitPrice = newPrice;
		}
